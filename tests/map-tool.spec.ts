...
async function waitForMapLoad(page: Page) {
    ...
    // The map is stored in appState.map, not window.map
    await page.waitForFunction(() => {
        const appState = (window as any).appState;
        if (!appState || !appState.map) return false;
        try {
            // Check if style is loaded (more reliable)
...
async function isCountrySelected(page: Page, countryName: string): Promise<boolean> {
    return await page.evaluate((name) => {
        ...
        return appState.selectedAreas.some((area: any) =>
            area.name && area.name.includes(name)
        );
    }, countryName);
}
...
test.describe('Area Type Selection', () => {
    ...
    test('should show admin level options when administration selected', async ({ page }) => {
        ...
        await expect(page.locator('[data-level="state"]')).toBeVisible();
        await expect(page.locator('[data-level="city"]')).toBeVisible();
        await expect(page.locator('[data-level="both"]')).toBeVisible();
    });
});
...
test.describe('Major Countries Selection', () => {
    ...
    test('should select United States', async ({ page }) => {
        ...
        if (selected) {
            ...
            expect(center.lng).toBeLessThan(-60);
    ...
    test('should select India', async ({ page }) => {
        ...
        if (selected) {
            ...
            const center = await getMapCenter(page);
            expect(center.lng).toBeGreaterThan(65);
    ...
    test('should select Brazil', async ({ page }) => {
        ...
        if (selected) {
            ...
            expect(center.lng).toBeLessThan(-30);
            expect(center.lat).toBeGreaterThan(-35);
            expect(center.lat).toBeLessThan(10);
        }
    });
    ...
    test('should select United Kingdom', async ({ page }) => {
        ...
        if (selected) {
            ...
            expect(center.lng).toBeLessThan(5);
    ...
    test('should select multiple major countries sequentially', async ({ page }) => {
        ...
        // Should have at least some areas selected
...
test.describe('Color Styling', () => {
    ...
    test('should change color using preset', async ({ page }) => {
        // Click on a color preset
        const redPreset = page.locator('.color-preset[data-color="#E05C5A"]');
...
// ============================================
// SECTION 7: BOUNDARY STYLE (Fill/Outline)
// ============================================
...
test.describe('AI Assistant', () => {
    ...
    test('should accept input in news textarea', async ({ page }) => {
        const textarea = page.locator('#news-input');
        await textarea.fill('这是一个关于北京的新闻报道。');

        await expect(textarea).toHaveValue('这是一个关于北京的新闻报道。');
    });
});
...
test.describe('Cross-Combination: Style × Color', () => {
    ...
    test('X07: Fill style + preset color', async ({ page }) => {
        ...
        // Select a color
        await page.locator('.color-preset[data-color="#496F96"]').click();
        await page.waitForTimeout(500);
        await expect(page.locator('.color-preset[data-color="#496F96"]')).toHaveClass(/active/);
    });
    ...
    test('X09: Outline style + preset color', async ({ page }) => {
        ...
        await page.locator('.color-preset[data-color="#EDBD76"]').click();
        await page.waitForTimeout(500);
        await expect(page.locator('.color-preset[data-color="#EDBD76"]')).toHaveClass(/active/);
    });
...
test.describe('Workflow: Manual Selection', () => {
    test('W02: Select multiple countries with different colors', async ({ page }) => {
        ...
        // Select color 1
        await page.locator('.color-preset[data-color="#6CA7A1"]').click();
        await page.waitForTimeout(500);

        // Select different color
        await page.locator('.color-preset[data-color="#E05C5A"]').click();
        await page.waitForTimeout(500);

        // If we get here, color switching works
        await expect(page.locator('.color-preset[data-color="#E05C5A"]')).toHaveClass(/active/);
    });
});
